...
Nachfolgend ein Beispiel für ein Minimalprogramm mit Einbindung des Zephyrkernels. Das Programm enthält die Konfiguration der LED Pinnummern fürs nRF52 Board. Passt man diese jedoch an, sollte es auf jeder Plattform lauffähig sein.
...
Demonstriert wird die Funktionalität der GPIOS und die Debugausgaben auf dem seriellen Terminal. Unter Linuxsystemen werden serielle Geräte, welche über einen \ac{FTDI} Chip verfügen, meistens unter /dev/ttyACM oder /dev/ttyUSB gemappt. Die Ausgaben sollten mittels seriellem Terminal under diesen Device Knoten lesbar sein. Die serielle Ausgabe von Zephyr erfolgt, sofern nichts anderes konfiguriert wurde, mit 1158200 Baud.
